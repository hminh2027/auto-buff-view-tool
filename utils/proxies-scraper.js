...
module.exports = async () => {
  ...
  const scrapped = types.map(async (t) => {
    const r = proxySites[t].map(async (s) => {
      const res = await axios.get(s).catch((e) => {
        console.log(e);
      });

      // const res = await needle("get", s, {
      //   response_timeout: 10000,
      //   follow_max: 5,
      //   rejectUnauthorized: false,
      // }).catch((e) =>
      //   logger.error(`Could not scrape proxies from ${s} : ${e}`)
      // );
  ...
  const proxies = await Promise.all(scrapped)
    .then((values) =>
      values.reduce((a, b) => a.concat(b.reduce((c, d) => c.concat(d), [])), [])
    )
